...
/// Kind of expression delimiter found in a template.
#[derive(Debug, Clone, Copy)]
pub enum ExpressionType {
    /// `{{! comment }}` or `{{!-- comment --}}`.
    Comment,
    /// Double-brace interpolation such as `{{name}}`.
    HtmlEscaped,
    /// Triple-brace interpolation such as `{{{html}}}`.
    Raw,
    /// A block opening such as `{{#if ready}}`.
    Open,
    /// A block closing such as `{{/if}}`.
    Close,
    /// Template text protected from expression parsing.
    Escaped,
}

/// A borrowed view of the next template expression and its surrounding text.
#[derive(Debug, Clone, Copy)]
pub struct Expression<'a> {
    /// The expression delimiter kind.
    pub expression_type: ExpressionType,
    /// Literal text before the expression.
    pub prefix: &'a str,
    /// Text inside the delimiters.
    pub content: &'a str,
    /// Unparsed template text after the expression.
    pub postfix: &'a str,
    /// The source slice used to produce this expression.
    pub raw: &'a str,
}
...
impl<'a> Expression<'a> {
    ...
    /// Parses the first expression in `src`.
    ///
    /// Returns `None` when the source contains no opening delimiter.
    ...
    /// Parses the next expression in [`Self::postfix`].
    pub fn next(&self) -> Result<Option<Self>> {
        Self::from(self.postfix)
    }

    /// Returns a short source fragment for diagnostics.
